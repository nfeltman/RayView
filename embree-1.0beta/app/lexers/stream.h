...
namespace embree
{
  ...
  public:
    ...
    ParseLocation (Ref<String> fileName, index_t lineNumber, index_t colNumber, index_t charNumber)
  ...
  private:
    ...
    index_t lineNumber;           /// the line number the token is from
    index_t colNumber;            /// the character number in the current line
    index_t charNumber;           /// the character in the file
  };
  ...
  public:
      ...
      : i(0), j(0), charNumber(0), name(new String(name))
    {
      ...
      for (index_t k=1; k<argc; k++) args.push_back(argv[k]);
    }
    ~CommandLineStream() {}
  public:
    ParseLocation location() {
