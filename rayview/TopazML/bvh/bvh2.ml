...
type tempBVH = (Box3.ne_box3, Box3.ne_box3 * Build_triangle.bTri array) Trees.tree
...
let measureCost root segs =
	let intersectRay seg : rayCost =
		let rec intersectNode node = match node with
			| Leaf(leaf) ->
					if Box3.ne_intersectsSeg leaf.boundsL seg then
						if Triangle.intersectsAny leaf.prims seg then
							Hits(1.0, 0.0)
						else
							Misses 1.0
					else
			...
			| Branch(branch, left, right) ->
					if not (Box3.ne_intersectsSeg branch.boundsB seg) then Misses 1.0 else
						...
						incCost(match
								...
								| LeftFirst -> 1.0
								| RightFirst -> 0.0
								| FrontToBack -> if firstIsCloser (center (bounds left)) (center (bounds right)) seg.originS then 1.0 else 0.0
								| BackToFront -> if firstIsCloser (center (bounds left)) (center (bounds right)) seg.originS then 0.0 else 1.0
								with
