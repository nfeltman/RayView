...
namespace embree
{
  HDRILight::HDRILight(const Parms& parms) : width(0), height(0), pixels(null)
  {
    ...
    for (size_t y = 0; y < height; y++)
      for (size_t x = 0; x < width; x++)
        importance.set(y, x, sinf(float(pi)*y/height) * reduce_add(pixels->get(x,y)));
  ...
  __forceinline Col3f HDRILight::Le(const Vec3f& wo) const
  {
    ...
    index_t x = clamp(index_t(u*width), index_t(0), index_t(width-1));
    index_t xNext = x + 1;
    if (size_t(xNext) == width) xNext = 0;
    float alpha = u*width - x;

    index_t y = clamp(index_t(v*height), index_t(0), index_t(height-1));
    index_t yNext = y+1;
  ...
  Col3f HDRILight::sample(const DifferentialGeometry& dg, Sample3f& wi, float& tMax, const Vec2f& sample) const
  {
    ...
    tMax = inf;
    return L*pixels->get(clamp(index_t(pixelF.value.x), index_t(0), index_t(width-1)),
                         clamp(index_t(pixelF.value.y), index_t(0), index_t(height-1)));
  }
