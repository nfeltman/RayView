...
namespace embree
{
  ...
  virtual ~ImageT() {
    ...
    __forceinline const T  operator()(size_t x, size_t y) const { return data[y*width+x]; }
    __forceinline       T& operator()(size_t x, size_t y)       { return data[y*width+x]; }
    __forceinline Col3f get(size_t x, size_t y) const           { return toCol3f(data[y*width+x]); }
    __forceinline void  set(size_t x, size_t y, const Col3f& c)        { setCol3f(data[y*width+x],c); }
    T* data;
  };
