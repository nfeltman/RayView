...
let measureCost root segs =
	let intersectRay seg =
		let rec measureCost node = match node with
			...
			| Branch(branch, left, right) -> if not (Box3.ne_intersectsSeg branch.boundsB seg) then (false, 0.0, 1.0) else
						let detEval firstChild secondChild =
							let (hits, c1, c2) = (measureCost firstChild) in
							if hits then (true, c1, c2) else (measureCost secondChild) in
						incCost( match
								...
								| pLeft ->
										let (lHits, lc1, lc2) = (measureCost left)in
										let (rHits, rc1, rc2) = (measureCost right) in
										(lHits || rHits, blendCost pLeft lc1 rc1, blendCost pLeft lc2 rc2))
		in measureCost root
	in let (spine, side, miss) = (ref 0.0, ref 0.0, ref 0.0) in
	Array.iter ( fun query -> ());
	{ spineCost = !spine; sideCost = !side; missCost = !miss }, { h_mh = 0 ; th_mm =0 ; tm_mh =0 ; tm_mm =0 }