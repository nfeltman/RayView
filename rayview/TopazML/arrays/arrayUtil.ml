...
let iterRange action (s, e) a =	for i = s to e - 1 do	action(a.(i)) done
...
let smartPartition filt notifier (st, e) arr =
	let rec part s k =
		if s >= e then k
			...
			let temp = arr.(st) in
			if filt(temp) then
				(arr.(st) <- arr.(k); arr.(k) <- temp; notifier temp k; notifier arr.(st) st;
					part (s +1) (k +1))
			else part (s +1) k
	in part st st
...
let pickMinFoldLeft objective seed arr =
	let e = Array.length arr in
